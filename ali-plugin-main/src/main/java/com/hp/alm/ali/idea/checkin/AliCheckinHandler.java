        ...
        if(markFixed.isSelected() || (addComment.isSelected() && !comment.getText().isEmpty())) {
            ...
            if(entity != null) {
                ...
                if(addComment.isSelected() && !comment.getText().isEmpty()) {
                    ...
                    String fontName = project.getComponent(AliProjectConfiguration.class).getFont();

                    String commentProperty = entity.isInitialized("dev-comments")? "dev-comments": "comments";
                    String mergedComment = CommentField.mergeComment(entity.getPropertyValue(commentProperty), comment.getText(), userName, fullName, fontName);
